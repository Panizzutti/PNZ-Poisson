...
#'
#' Computes the cumulative distribution function (CDF) of the PNZ-Poisson distribution for a vector of integer values. The parameters `lambda` and `theta` can also be vectors of the same length as `q`, or any parameter can be a single value to be applied across all evaluations.
#'
...
#' ppnzpois(0:10, lambda = 5, theta = 6)
#'
#' @export
ppnzpois <- function(q, lambda, theta) {
  ...
  sapply(seq_along(q), function(i) {
    ppnzpois_scalar(q[i], lambda[i], theta[i])
  })
...
#'
#' Computes the probability mass function (PMF) of the PNZ-Poisson distribution for a vector of integer values. The parameters \code{lambda} and \code{theta} can also be vectors of the same length as \code{k}, or any parameter can be a single value to be applied across all evaluations.
#'
...
#' dpnzpois(0:5, lambda = 2, theta = c(1, 1.1, 1.2, 1.3, 1.4, 1.5))
#'
#' # Scalar lambda and theta with vector k
#' dpnzpois(0:5, lambda = 2, theta = 1)
#'
#' @export
dpnzpois <- function(k, lambda, theta) {
  # Get the lengths of each input
  ...
  sapply(seq_along(k), function(i) {
    dpnzpois_scalar(k[i], lambda[i], theta[i])
  })
}
...
#'
#' Computes the quantile function of the PNZ-Poisson distribution for a vector of probabilities. The parameters \code{lambda} and \code{theta} can also be vectors of the same length as \code{p}, or any parameter can be a single value to be applied across all evaluations.
#'
...
#' qpnzpois(c(0.25, 0.5, 0.75), lambda = 10, theta = 1)
#'
#' # Vectorized parameters: different values for theta
#' qpnzpois(c(0.25, 0.5, 0.75), lambda = c(1, 5.3, 6.5), theta = 8)
#'
#' # Scalar values
#' qpnzpois(0.99, lambda = 24, theta = 1)
#'
#' @export
qpnzpois <- function(p, lambda, theta, lower.tail = TRUE, log.p = FALSE) {
  ...
  sapply(seq_along(p), function(i) {
    ...
    k <- 0
    while (ppnzpois(k, current_lambda, current_theta) < current_p) {
      k <- k + 1
...
#' lambda <- 2
#' theta <- 1
#' log_likelihood <- logLikPNZ(k_obs, lambda, theta)
#' print(log_likelihood)
#'
#' # Vector of observed counts with scalar lambda and theta
#' k_obs <- 0:5
#' log_likelihood <- logLikPNZ(k_obs, lambda = 2, theta = 1)
#' print(log_likelihood)
#'
...
#'
#' @export
logLikPNZ <- function(k, lambda, theta) {
  # Get the lengths of each input
  lengths <- c(length(k), length(lambda), length(theta))
